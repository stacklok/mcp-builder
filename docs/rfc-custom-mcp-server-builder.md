...
A repeatable, four-phase pipeline that transforms an OpenAPI spec and plain-English workflow descriptions into a production-quality, ToolHive-ready MCP server. Phase 1 (AI Scoping) uses AI skill files to parse the spec, cluster endpoints by workflow, assign LLM-optimized tool names and descriptions, and emit a single `mcp-scope.yaml` config. Phase 2 (Human Review) is an edit pass over that YAML. Phase 3 (Deterministic Code Generation) reads the YAML and scaffolds a complete MCP server from the `py-mcp-template` — no AI involved. Phase 4 (AI Validation) reviews the generated code, builds the server, and smoke-tests it against the real upstream API. The first proof point is a Google Drive MCP server deployed in North's cluster.
...
Open-source proxies like `mcp-openapi-proxy` prove the concept but are not production quality — they pass through the full spec surface, offer no curation, and generate no ToolHive deployment artifacts.

### Who Is Affected

- **North's platform team**: They need custom MCP servers for internal services (Google Drive, BambooHR, Harvest, Greenhouse) and lack the tooling to build them repeatably.
- **Any team adopting ToolHive**: The same problem exists for every enterprise API without a first-party MCP server. The builder makes this a commodity operation rather than bespoke engineering.

### Why Worth Solving

North's POC deliverable includes both a working MCP server *and* the ability to independently repeat the process. A one-off server build doesn't compound — a repeatable pipeline does. This also establishes the pattern for future ToolHive ecosystem growth: as customers adopt ToolHive, they will need to wrap internal APIs that will never have community MCP servers.
...
- **Runtime proxy**: This pipeline generates static, compiled MCP servers — not a runtime proxy that interprets the spec at request time.
...
The pipeline is intentionally split at the `mcp-scope.yaml` boundary. Everything to the left of it involves judgment (AI + human). Everything to the right is mechanical (deterministic codegen). A user who wants to skip AI entirely can hand-write the YAML and enter at Phase 3.
...
2. **Endpoint Clustering by Workflow**
   - Take all workflow descriptions as input simultaneously.
   - Cluster endpoints into capability groups matched to the described workflows. A capability group is a set of related endpoints that together support a workflow (e.g., "Document Management" might include `files.list`, `files.get`, `files.create`, `files.copy`, `files.delete`).
   - Each endpoint belongs to at most one group. Endpoints that don't match any workflow are placed in an "Uncategorized" group for human review.
   - Present the groups to the user for approval before proceeding.

3. **Tool Naming**
   - Assign a clean, LLM-friendly tool name to each selected endpoint. Names follow the pattern `verb_noun` (e.g., `list_files`, `create_document`, `search_contacts`).
...
4. **Description Writing**
   - Write an LLM-optimized description for each tool. Descriptions target the LLM as the reader — they encode what the tool does, when to use it, what it returns, and any non-obvious constraints.
   - Write an LLM-optimized description for each parameter. Include default values, valid ranges, enum values, and behavioral notes that the spec's description may omit.
...
| No endpoints match any workflow | Skill presents all endpoints as uncategorized for manual selection |
...
- Parameters listed override the spec's descriptions. Parameters not listed use the spec's description as-is.
...
├── Dockerfile                  # Multi-stage build, ToolHive-compatible
├── src/
│   └── google_drive_mcp/
│       ├── __init__.py
...
│       ├── tools/
│       │   ├── __init__.py
│       │   ├── file_operations.py   # One module per group
│       │   └── permissions.py
│       ├── models/
│       │   ├── __init__.py
│       │   └── schemas.py      # Pydantic models from OpenAPI schemas
│       └── client.py           # Async HTTP client with token passthrough
└── deploy/
    ├── mcpserver.yaml          # ToolHive MCPServer CRD manifest
    ├── mcpexternalauthconfig.yaml  # Auth config (if applicable)
    └── secret.yaml             # Template for K8s Secret (credentials placeholder)
```
...
**Determinism guarantee**: The generator is a pure function of its inputs. No randomness, no network calls, no AI. Running it twice on the same `mcp-scope.yaml` + spec produces byte-identical output (modulo timestamps in generated comments, if any).
...
- **Multi-call tools** (often hint-driven): If two endpoints are commonly used together (e.g., get file metadata + download content), suggest combining them into a single tool.
...
| Smoke test returns API error | Warning | May be expected (e.g., missing test data). Human reviews. |
...
- **Skill regression tests**: Maintain a set of OpenAPI specs (varying quality levels) and golden `mcp-scope.yaml` outputs. Re-run the AI skill periodically and diff against the golden files to catch regressions in skill instructions.
...
2. **Parameter description depth**: How aggressive should the AI be in rewriting parameter descriptions for bad specs? Should it hallucinate plausible defaults, or leave them empty and flag for human review?
3. **Multi-server composition**: When a customer needs tools from multiple APIs (e.g., Google Drive + Google Calendar), should the pipeline produce separate servers that are grouped via ToolHive vMCP, or a single server with multiple upstreams? This interacts with ToolHive's vMCP and group features.
4. **Template repo coupling**: Should `py-mcp-template` be vendored into this repo or referenced as a separate dependency? Vendoring simplifies versioning; a separate repo allows independent evolution.
...
- [MCP OpenAPI Proxy (community)](https://github.com/matthewhand/mcp-openapi-proxy) — reference for the "naive wrapping" approach
