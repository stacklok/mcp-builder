...
different requirements:
    ...
    spec (mcp_builder.spec) — strict extraction for the codegen pipeline.
        Extracts only the endpoints/parameters the scope selects, and raises
        on anything unresolvable. Bad input to code generation must fail fast.
...
Reading order:
    1. spec_analyzer — surveys the full spec for the ``analyze`` command
